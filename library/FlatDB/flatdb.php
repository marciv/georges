    ...
    public function __construct($data_path, $db = 'default') {
        ...
        if(!is_dir($this->data_dir)) {
            ...
            if(!mkdir($this->data_dir)) {
				...
				file_put_contents($this->data_dir . 'index.php', '<?php //Silence is golden 1');
				mkdir($this->data_dir.'/data_set');
				file_put_contents($this->data_dir .'/data_set/'. 'index.php', '<?php //Silence is golden 2');
			}
        }
    }
    ...
    public function insert($obj) {
        ...
        if(!is_dir($this->data_dir . $table)) {
            ...
            if(!@mkdir($this->data_dir . $table, 0777)) {
				...
				// Add empty index.php file if directory has been created successfully
				file_put_contents($this->data_dir . $table . '/' . 'index.php', '<?php //Silence is golden 2');
			}
